...
pub async fn get_nonce(address: &str, chain_id: u64) -> Result<u64, String> {
    let chains = get_default_chains();
    let chain = chains
        .iter()
        .find(|c| c.chain_id == chain_id)
...
pub async fn get_block_number(chain_id: u64) -> Result<u64, String> {
    let chains = get_default_chains();
    let chain = chains
        .iter()
        .find(|c| c.chain_id == chain_id)
...
pub fn get_chain_config(chain_id: u64) -> Option<ChainConfig> {
    get_default_chains().into_iter().find(|c| c.chain_id == chain_id)
}

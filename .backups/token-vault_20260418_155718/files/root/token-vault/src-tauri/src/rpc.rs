...
pub async fn get_native_balance(address: &str, chain_id: u64) -> Result<BalanceInfo, String> {
    ...
    let balance_formatted = if fraction == 0 {
        format!("{} {}", whole, chain.symbol)
    } else {
        format!("{}.{:0>18} {}", whole, fraction, chain.symbol)
    };
...
/// 
/// # Arguments
/// * `signed_transaction_hex` - The signed transaction as a hex string (with or without 0x prefix)
/// * `chain_id` - The chain ID to send the transaction on
/// 
/// # Returns
/// * `Ok(String)` - The transaction hash
/// * `Err(String)` - Error message if the transaction failed
pub async fn send_raw_transaction(
    ...
    let normalized_hex = if signed_transaction_hex.starts_with("0x") {
        signed_transaction_hex.to_string()
    } else {
        format!("0x{}", signed_transaction_hex)
    };
    
    let request = JsonRpcRequest::new(
        "eth_sendRawTransaction",
        vec![serde_json::Value::String(normalized_hex)],
    );
    ...
    if let Some(error) = result.get("error") {
        ...
        return Err(format!("Transaction failed: {}", error_message));
    }

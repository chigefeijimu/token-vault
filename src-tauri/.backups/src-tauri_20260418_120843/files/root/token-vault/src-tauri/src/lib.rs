// TokenVault - Tauri 2.x Backend Library
...
async fn create_wallet(
    ...
    // INTRODUCED ERROR for testing
    wallet_manager.create_wallet(&name, &password, "extra_arg")
...
async fn export_keystore(
    ...
    let wallet_manager = state.wallet_manager.read();
    wallet_manager.export_private_key(&wallet_id, &password)
        .map_err(|e| e.to_string())
}
